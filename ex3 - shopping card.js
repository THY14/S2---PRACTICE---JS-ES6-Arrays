...
function getCartTotalAmount() {
  ...
  SHOPPING_CART.forEach((product) => {
    const productInShop = PRODUCTS.find((p) => p.id === product.id);
    if (productInShop) {
      total += productInShop.price * product.quantity;
    }
  });
  return total;
...
function removeProductFromCart(productId) {
  ...
  if (product && product.quantity > 1) {
    ...
    console.error(`Product with id ${productId} not found in the shopping cart.`);
  }
}
